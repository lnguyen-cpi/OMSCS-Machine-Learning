...
import hiive_local.visualization.mdpviz.next_state
import hiive_local.visualization.mdpviz.reward
from hiive_local.visualization import mdpviz
from hiive_local.visualization.mdpviz.dsl import context
...
class Reward(Node):
    def __init__(self, reward: hiive_local.visualization.mdpviz.reward.Reward):
        super().__init__()

        self.reward = reward

    def __mul__(self, other):
        return Reward(hiive_local.visualization.mdpviz.reward.Reward(self.reward.outcome, self.reward.weight * other))
...
class State(Node, SupportConjunction, SupportMapping):
    ...
    def __mul__(self, other):
        return WeightedState(hiive_local.visualization.mdpviz.next_state.NextState(self.state, other))
...
class WeightedState(Node):
    def __init__(self, next_state: hiive_local.visualization.mdpviz.next_state.NextState):
        super().__init__()
    ...
    def __mul__(self, other):
        return WeightedState(
            hiive_local.visualization.mdpviz.next_state.NextState(self.next_state.outcome, self.next_state.weight * other))
...
class TransitionOutcomeVisitor(NodeVisitor):
    ...
    def visit_state(self, node: State):
        for transition in self.iterator:
            yield transition.replace(outcome=hiive_local.visualization.mdpviz.next_state.NextState(node.state))
